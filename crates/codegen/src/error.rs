...
use eth_ir_data::{BasicBlockId, CasesId, DataId, FunctionId, LargeConstId, LocalId};
...
pub enum CodegenError {
    ...
    InvalidFunctionReference { function: FunctionId },
    /// Stack depth exceeded for local variable access
...
impl fmt::Display for CodegenError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            CodegenError::InvalidFunctionReference { function } => {
                write!(f, "Invalid function reference {:?}", function)
            }
            CodegenError::StackTooDeep { depth } => {
                write!(f, "Stack too deep: cannot access local at depth {}", depth)
            }
        }
    }
}
